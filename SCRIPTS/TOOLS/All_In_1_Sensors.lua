...
local labels = {
  ...
  "RPM",
  "RSSI",
  "Température processeur FBL",
  "Temp ESC",
  "Conso mAh, capteur à créer (consumpt)pour capacité",
  "VFR (Valide Frame Rate)",
  "CHP/Occupation Proc FBL temps reel"
}

local keys = {
  "VBAT","VBEC","CURR","RPM","RSSI","TFBL","TESC","CONS","VFR","CHP"
}
...
-- Fonction pour sauvegarder la configuration avec gestion d'erreur améliorée
local function saveCfg()
  ...
  if success then
    savedMessage = "Sauvegarde OK!"
  else
    savedMessage = "ERREUR sauvegarde!"
  end
...
local function run(event)
  ...
  -- Fond noir
  lcd.drawFilledRectangle(0, 0, LCD_W, LCD_H, lcd.RGB(0, 0, 0))
  
  -- Couleurs
  local whiteColor = lcd.RGB(255, 255, 255)
  local redColor = lcd.RGB(255, 0, 0)
  local darkGrey = lcd.RGB(40, 40, 40)
  
  -- Titre
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawText(LCD_W / 2, 5, "Config capteurs All_IN_1", MIDSIZE + CENTER + CUSTOM_COLOR)
  
  -- Ligne de séparation
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawLine(0, 32, LCD_W, 32, CUSTOM_COLOR, 1)
  
  -- Instructions
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  if editMode then
    lcd.drawText(LCD_W / 2, 38, "Molette=choisir  ENTER=OK  EXIT=annuler", SMLSIZE + CENTER + CUSTOM_COLOR)
  else
    lcd.drawText(LCD_W / 2, 38, "Haut/Bas=naviguer  ENTER=modifier  EXIT=quitter", SMLSIZE + CENTER + CUSTOM_COLOR)
  end
  ...
  elseif event == EVT_VIRTUAL_ENTER or event == EVT_ENTER_BREAK then
    if editMode then
      ...
      if selectedSource then
        ...
        local saveSuccess = saveCfg()
        if not saveSuccess then
          -- En cas d'échec, afficher en rouge
          savedMessage = "ERREUR sauvegarde!"
        end
      end
      editMode = false
  ...
  -- Zone de liste
  local listY = 60
  local listH = LCD_H - listY - 39  -- MODIFIÉ: augmenté pour plus d'espace en bas
  local lineH = 28
  local maxLines = 5
  
  -- Dessiner le cadre de la liste
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawRectangle(0, listY, LCD_W, listH, CUSTOM_COLOR, 1)
  
  -- En-têtes de colonnes
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawText(15, listY + 0, "Capteur", SMLSIZE + BOLD + CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawText(LCD_W - 110, listY + 0, "Source", SMLSIZE + BOLD + CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR, whiteColor)
  lcd.drawLine(5, listY + 26, LCD_W - 5, listY + 26, CUSTOM_COLOR, 1)
  ...
  for i = startLine, endLine do
    ...
    if i == selectedLine then
      lcd.setColor(CUSTOM_COLOR, whiteColor)
      lcd.drawRectangle(5, y - 4, LCD_W - 10, lineH - 2, CUSTOM_COLOR, 2)
    end
    
    -- Label
    lcd.setColor(CUSTOM_COLOR, whiteColor)
    lcd.drawText(15, y, label, SMLSIZE + CUSTOM_COLOR)
    
    -- Valeur
    if editMode and i == selectedLine then
      local editName = sourcesList[editIndex].name
      lcd.setColor(CUSTOM_COLOR, whiteColor)
      lcd.drawRectangle(LCD_W - 120, y - 2, 110, lineH - 6, CUSTOM_COLOR, 1)
      lcd.setColor(CUSTOM_COLOR, whiteColor)
      lcd.drawText(LCD_W - 115, y, editName, SMLSIZE + BOLD + INVERS + CUSTOM_COLOR)
    else
      lcd.setColor(CUSTOM_COLOR, whiteColor)
      lcd.drawText(LCD_W - 115, y, value, SMLSIZE + CUSTOM_COLOR)
    end
    
    y = y + lineH
  end
  
  -- Barre d'état (MODIFIÉ: hauteur augmentée et position du texte ajustée)
  lcd.drawFilledRectangle(0, LCD_H - 32, LCD_W, 32, darkGrey)
  
  if savedMessage ~= "" and (getTime() - savedMessageTime) < 150 then
    -- Afficher le message avec la couleur appropriée
    local msgColor = (string.find(savedMessage, "ERREUR") ~= nil) and redColor or whiteColor
    lcd.setColor(CUSTOM_COLOR, msgColor)
    lcd.drawText(5, LCD_H - 26, savedMessage, SMLSIZE + BOLD + CUSTOM_COLOR)
  else
    savedMessage = ""
    lcd.setColor(CUSTOM_COLOR, whiteColor)
    lcd.drawText(5, LCD_H - 26, string.format("Sources: %d", #sourcesList), SMLSIZE + CUSTOM_COLOR)
  end
  
  if editMode then
    lcd.setColor(CUSTOM_COLOR, whiteColor)
    lcd.drawText(LCD_W - 5, LCD_H - 26, string.format("%d/%d", editIndex, #sourcesList), SMLSIZE + RIGHT + CUSTOM_COLOR)
  else
    lcd.setColor(CUSTOM_COLOR, whiteColor)
    lcd.drawText(LCD_W - 5, LCD_H - 26, string.format("Ligne %d/%d", selectedLine, #keys), SMLSIZE + RIGHT + CUSTOM_COLOR)
  end
